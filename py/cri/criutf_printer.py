# prints CRI @UTF info
import glob, struct, os, argparse
...
class CriPrinter():
    def __init__(self, print_stdout=False):
        self._indent = 0
        self._lines = []
    ...
    def info(self, text):
        ...
        self._lines.append(padding + text)

    

    def _parse_utf(self, data):
        utf = CriUtf(data)

        for row_pos in range(utf.rows):
            self.next( f"{utf.name}[{row_pos}] = {{" )

            for col_pos in range(utf.columns):
                col = utf.get_column_schema(col_pos)
                if not col:
                    continue
                row = utf.query_pos(row_pos, col_pos)

                col_info = str(col)
                row_info = str(row)

                if col.type == CriUtf.COLUMN_TYPE_VLDATA:
                    offset, size = row
                    row_info = f'[{offset:08x} + {size:08x}]'

                self.info(f'{col_info} = {row_info}')

                if col.type == CriUtf.COLUMN_TYPE_VLDATA:
                    offset, size = row
                    if size:
                        subdata = data[offset : offset + size]
                        if subdata[0:4] == b'@UTF':
                            self.next("{")
                            self._parse_utf(subdata)
                            self.prev( "}" )
                
            self.prev("}")
            

    def _read_utf_data(self, f, offset=0):
        f.seek(offset)
        data = f.read(0x08)
        ...
        if data[0:4] != b'@UTF':
            return None
        size = struct.unpack_from('>I', data, 0x04)[0]
        data += f.read(size)
        return data


    # include new file
    def parse_file(self, filename, offset=0):
        with open(filename, 'rb') as f:
            data = self._read_utf_data(f, offset)
            if not data:
                print(f"not an utf file: {filename}")
                return

            self.next(filename)
            self._parse_utf(data)
            self.prev('')
...
def parse():
    description = (
        "writes CRI @UTF table info"
    )

    parser = argparse.ArgumentParser(description=description, formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("files", help="files to get (wildcards work)", nargs='*', default=["*.acb", "*.usm"])
    parser.add_argument("-o","--offset", help="table offset", default=0)
    parser.add_argument("-p","--print-stdout", help="prints instead of writting to file", action='store_true')
...
def main():
    ...
    for filename in filenames:
        if not os.path.isfile(filename):
            continue
        cri = CriPrinter(args.print_stdout)
        cri.parse_file(filename, args.offset)

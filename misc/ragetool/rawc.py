...
class AwcParser():
    ...
    def parse_tag(self, type, offset, size, is_seek_samples):
        pos = self._r.pos() #save

        self._r.pos(offset) #save
        self.next()
        ...
        if   type == 0x55:
            ...
            for ch in range(channels):
                ...
                codec_info = rdefs.AWC_CODECS.get(codec, '?')
            ...
            self._r.pos(offset + size)
            self.prev()

        self.prev()
        self._r.pos(pos) #restore

    def parse(self):
        self.add(f'{self.filename}').next()

        id = self.fourcc();
        if   id == b'ADAT':
            self._r.set_be(False)
        elif id == b'TADA':
            self._r.set_be(True)
        else:
            raise ValueError('not an AWC file')
